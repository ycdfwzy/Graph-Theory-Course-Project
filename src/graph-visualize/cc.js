...
$('#Compute').click((event) => {
    ...
    nprogress.start();
    cpp.connected_component(threshold, render);
}).click();

//如果dll当前正在工作，就不进行页面的跳转
$("a").click((event) => {
    if (shared.working) {
        event.preventDefault();
    }
...
function render(error, graph) {
    ...
    const link = link_g.selectAll("line")
        .attr("stroke-width", function (d) {//设置线宽
    ...
    const node = node_g.selectAll("circle")
        ...
        .on("click", function (d) {
            ...
            $('#information').html('电影：' + d.id + '<br/>所在分量：' + d.group);
        });
    ...
    $('#total_count').text(graph.count);//显示附加信息
    $('#more_than_one').text(graph.more_than_one);
